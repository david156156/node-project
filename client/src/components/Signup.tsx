...
const Signup: FunctionComponent<SignupProps> = () => {
  ...
  const validationSchema = yup.object({
    name: yup.object({
      first: yup.string().min(2).max(255).required("First name is required"),
      middle: yup.string().max(255),
      last: yup.string().min(2).max(255).required("Last name is required"),
    }),
    phone: yup.string().min(9).max(14).required("Phone is required"),
    email: yup
      .string()
      .min(6)
      .email("Invalid email")
      .required("Email is required"),
    password: yup.string().min(6).max(1024).required("Password is required"),
    image: yup.object({
      url: yup.string(),
